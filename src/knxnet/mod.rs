...
impl Service {
    ...
    pub(crate) fn decoded(buf: &[u8]) -> Result<Service, KnxNetIpError> {
        if buf.len() < 6 {
...
mod tests {
    ...
    fn t_service_encode() {
        ...
        assert_eq!(Service::DisconnectResponse(DisconnectResponse {
            channel: 21,
            status: StatusCode::NoError
        }).encoded(), vec![0x06, 0x10, 0x02, 0x0A, 0x00, 0x08, 0x15, 0x00]);
    }
